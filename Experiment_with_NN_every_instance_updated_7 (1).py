    ...
    while (i + window < len(data)):
        ...
        for (count, var) in enumerate(Xtmp):
            ...
            if not np.isnan(var[-1]):
                ...
                # print(unlabel_count)
                unlabeled_instances.append(unlabel_count + 1)
                ...
                features = np.dstack([
                    ...
                    (pred_NN_3 == pred_NN_5).astype(int),distance_NN_mean_7,
                    np.max(proba_NN_7, axis=1),
                    (pred_NN_5 == pred_NN_7).astype(int)])
                # print(features[0])
                ...
                if np.count_nonzero(critic_predition == 4) > 0:
                    # add_x2 = X_unlabled_array
                    # print(y_prevwindow_array)
                    # add_y2 = y_prevwindow_array.astype(int)[indices][0]
                    counts[4]+=1
...
def experiment_all_NN_every_instance_7(data, classes1, missing_rates, semi_approach='proba', m=10,
